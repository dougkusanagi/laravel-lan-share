...
final class SharePageController
{
    ...
    public function __invoke(Request $request): Response
    {
        ...
        $localHomeUrl = $this->links->applicationUrl($localBaseUrl);
        ...
        $pairingTtl = max(0, (int) config('lan-share.pairing.token_ttl', 180));
        $pairingExpiryLabel = $pairingTtl === 0
            ? 'Uso único, sem expiração'
            : 'Expira em '.(int) ceil($pairingTtl / 60).' min';
        ...
        $loginUrl = rtrim($localBaseUrl, '/').'/'.$pagePath.'/login';
...
<style>.system-back{display:inline-flex;align-items:center;gap:7px;margin:0 0 20px;color:#68736f;font-size:12px;font-weight:760;text-decoration:none;transition:color .18s,transform .18s}.system-back:hover{color:#c94331;transform:translateX(-2px)}.system-back svg{width:15px;height:15px}</style>
<style>.pairing-share{display:flex;align-items:center;justify-content:space-between;gap:15px;margin-top:18px;padding-top:18px;border-top:1px solid #edf0eb}.pairing-share-copy{min-width:0}.pairing-url{display:block;max-width:310px;overflow:hidden;color:#53615a;font-size:12px;text-overflow:ellipsis;white-space:nowrap}.pairing-actions{display:flex;flex:none;gap:7px}.pairing-actions .whatsapp-button{color:#257253}.pairing-actions .whatsapp-button:hover{border-color:#b9d8ca;color:#257253;background:#eef8f1}@media (max-width:560px){.pairing-share{align-items:flex-start}.pairing-actions{padding-top:2px}}</style>
</head>
<body>
<main class="page">
...
<a class="system-back" href="{$e($localHomeUrl)}"><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" aria-hidden="true"><path d="m12 19-7-7 7-7"/><path d="M19 12H5"/></svg>Voltar pro sistema</a><section class="hero"><div class="hero-copy"><p class="eyebrow"><span class="eyebrow-line"></span>Compartilhamento local</p><h1>Leve seu projeto <em>com você.</em></h1><p class="hero-lede">Conecte o celular ao ambiente de desenvolvimento em segundos. Um QR Code de uso único, uma sessão segura e zero endereço para digitar.</p><div class="feature-row"><span class="feature"><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.8" stroke-linecap="round" stroke-linejoin="round" aria-hidden="true"><path d="M12 3 5 6v5c0 4.5 2.9 8.3 7 10 4.1-1.7 7-5.5 7-10V6l-7-3Z"/><path d="m9.4 12 1.7 1.7 3.7-3.7"/></svg>Token de uso único</span><span class="feature"><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.8" stroke-linecap="round" stroke-linejoin="round" aria-hidden="true"><circle cx="12" cy="12" r="9"/><path d="M12 7v5l3 2"/></svg>{$e($pairingExpiryLabel)}</span></div></div>
...
<footer class="footer"><span>Uma experiência de desenvolvimento local do {$e($project)}</span></footer>
</main>
<script>
...
HTML;
    ...
    private function pairingMarkup(bool $isAuthenticated, bool $pairingEnabled, callable $escape, string $loginUrl): string
    {
        ...
        if (! $isAuthenticated) {
            return '<div class="pairing-panel"><div class="guest"><p>Entre neste computador para liberar o acesso rápido e gerar um QR Code de uso único.</p><a class="button button-ghost" href="'.$escape($loginUrl).'">Entrar <span aria-hidden="true">↗</span></a></div></div>';
        }

        return <<<'HTML'
<div class="pairing-panel" id="pairing-panel">
<div id="pairing-locked"><div class="pairing-copy"><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.8" stroke-linecap="round" stroke-linejoin="round" aria-hidden="true"><rect width="16" height="11" x="4" y="10" rx="2"/><path d="M8 10V7a4 4 0 0 1 8 0v3"/><path d="M12 14v3"/></svg><div><h3>Autorize um novo dispositivo</h3><p>Confirme sua senha para liberar um QR Code de uso único.</p></div></div><form class="pairing-form" id="pairing-form"><label class="field-label" for="pairing-password">Sua senha</label><div class="input-row"><input class="input" id="pairing-password" name="password" type="password" autocomplete="current-password" placeholder="Digite sua senha" required><button class="button button-primary" type="submit" id="pairing-submit"><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.8" stroke-linecap="round" stroke-linejoin="round" aria-hidden="true"><rect width="16" height="11" x="4" y="10" rx="2"/><path d="M8 10V7a4 4 0 0 1 8 0v3"/></svg>Gerar link de acesso</button></div><p class="helper">A senha é validada somente nesta sessão e nunca é armazenada.</p><p class="pairing-error" id="pairing-error"></p></form></div>
<div class="pairing-active hidden" id="pairing-active"><div class="qr-shell" id="qr-shell"><div id="qr-surface"></div><div class="qr-overlay" id="qr-overlay" hidden><div class="qr-overlay-inner"><span class="qr-overlay-icon"><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.8" stroke-linecap="round" stroke-linejoin="round" aria-hidden="true"><path d="M12 3 5 6v5c0 4.5 2.9 8.3 7 10 4.1-1.7 7-5.5 7-10V6l-7-3Z"/><path d="M12 8v4M12 16h.01"/></svg></span><span id="qr-overlay-text">QR Code encerrado</span></div></div></div><div class="active-meta"><span class="active-status"><span class="active-status-dot"></span><span id="pairing-status">Aguardando leitura</span></span><p class="active-help" id="pairing-help">Abra a câmera do celular e aponte para este código.</p><p class="timer" id="pairing-timer"></p></div><div class="pairing-share hidden" id="pairing-share"><div class="pairing-share-copy"><p class="manual-kicker">Link de login automático</p><code class="pairing-url" id="pairing-url"></code></div><div class="pairing-actions"><button class="icon-button" id="pairing-copy" type="button" title="Copiar link de login" aria-label="Copiar link de login"><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" aria-hidden="true"><rect width="14" height="14" x="8" y="8" rx="2"/><path d="M4 16c-1.1 0-2-.9-2-2V4c0-1.1.9-2 2-2h10c1.1 0 2 .9 2 2"/></svg></button><a class="icon-button whatsapp-button" id="pairing-whatsapp" href="#" target="_blank" rel="noopener noreferrer" title="Enviar link de login pelo WhatsApp" aria-label="Enviar link de login pelo WhatsApp"><svg viewBox="0 0 24 24" fill="currentColor" aria-hidden="true"><path d="M20.5 3.5A11.8 11.8 0 0 0 12.1 0C5.6 0 .3 5.3.3 11.8c0 2.1.6 4.1 1.6 5.9L.2 24l6.5-1.7a11.8 11.8 0 0 0 5.4 1.3h.1c6.5 0 11.8-5.3 11.8-11.8 0-3.1-1.2-6-3.5-8.3ZM12.1 21.6c-1.7 0-3.4-.5-4.9-1.3l-.4-.2-3.9 1 1-3.8-.3-.4a9.7 9.7 0 0 1-1.5-5.2c0-5.4 4.4-9.8 9.9-9.8 2.6 0 5.1 1 6.9 2.9a9.7 9.7 0 0 1 2.9 6.9c0 5.5-4.5 9.9-9.9 9.9Zm5.4-7.4c-.3-.2-1.8-.9-2.1-1-.3-.1-.5-.2-.7.2-.2.3-.8 1-.9 1.2-.2.2-.3.2-.6.1-1.6-.8-2.7-1.4-3.8-3.2-.3-.5.3-.4.8-1.4.1-.2.1-.4 0-.5 0-.2-.7-1.7-.9-2.3-.2-.6-.5-.5-.7-.5h-.6c-.2 0-.5.1-.8.4-.3.3-1 1-1 2.5s1 2.9 1.1 3.1c.1.2 2 3.1 4.8 4.3 1.8.8 2.5.9 3.4.8.5-.1 1.8-.7 2-1.3.3-.6.3-1.2.2-1.3-.1-.2-.3-.3-.6-.4Z"/></svg></a></div></div><button class="button button-reset" id="pairing-reset" type="button">Gerar outro QR Code</button></div>
</div>
HTML;
    }
    ...
    private function pairingScript(bool $enabled, string $csrfToken, string $pagePath, string $target): string
    {
        ...
        return <<<JS
const pairingForm=document.querySelector('#pairing-form'),pairingSubmit=document.querySelector('#pairing-submit'),pairingPassword=document.querySelector('#pairing-password'),pairingError=document.querySelector('#pairing-error'),pairingLocked=document.querySelector('#pairing-locked'),pairingActive=document.querySelector('#pairing-active'),qrShell=document.querySelector('#qr-shell'),qrSurface=document.querySelector('#qr-surface'),qrOverlay=document.querySelector('#qr-overlay'),qrOverlayText=document.querySelector('#qr-overlay-text'),pairingStatus=document.querySelector('#pairing-status'),pairingHelp=document.querySelector('#pairing-help'),pairingTimer=document.querySelector('#pairing-timer'),pairingShare=document.querySelector('#pairing-share'),pairingUrl=document.querySelector('#pairing-url'),pairingCopy=document.querySelector('#pairing-copy'),pairingWhatsApp=document.querySelector('#pairing-whatsapp'),pairingReset=document.querySelector('#pairing-reset'),pairingTarget={$target};
...
function updateCountdown(expiresAt){if(!expiresAt){pairingTimer.textContent='Sem expiração';return}const tick=()=>{const remaining=Math.max(0,expiresAt-Math.floor(Date.now()/1000));pairingTimer.textContent=remaining>0?'Expira em '+remaining+'s':'Expirando…';if(remaining<=0){finishPairing('expired')}};tick();countdownTimer=setInterval(tick,1000)}
